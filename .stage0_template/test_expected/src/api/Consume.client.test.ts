...
describe('API Client - Consume Endpoints', () => {
  ...
  it('should get all consumes', async () => {
    const mockItems = [
      { _id: '507f1f77bcf86cd799439011', name: 'test-consume', description: 'Test', status: 'active' }
    ]
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/consume', expect.any(Object))
  })
  ...
  it('should get consumes with name query', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/consume?name=test', expect.any(Object))
  })

  it('should get a single consume', async () => {
    const mockItem = { _id: '507f1f77bcf86cd799439011', name: 'test-consume' }
