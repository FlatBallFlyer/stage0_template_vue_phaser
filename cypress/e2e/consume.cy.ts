describe('Consume Domain', () => {
  ...
  it('should search for consumes using existing test data', () => {
    ...
    cy.get('body').then(($body) => {
      ...
      if (hasRows) {
        ...
        cy.get('table tbody tr').first().find('td').first().invoke('text').then((firstConsumeName) => {
          const trimmedName = firstConsumeName.trim()
  ...
  // it('should load more consumes with infinite scroll', () => {
  //   cy.visit('/consumes')
  //   
  //   // Wait for initial load
  //   cy.get('table').should('exist')
  ...
  //   cy.get('body').then(($body) => {
  //     if ($body.find('[data-automation-id="consume-list-load-more"]').length > 0) {
  //       // Click load more if available
  //       cy.get('[data-automation-id="consume-list-load-more"]').click()
  //       cy.wait(1000)
