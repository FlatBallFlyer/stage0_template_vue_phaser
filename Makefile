...
help:
	@echo "  make test             - Run tests using ~/tmp/testRepo folder"
	@echo "  make clean            - Clean up temporary test files"
	@echo "  make merge <specs_path>  - Merge templates (repo is .)"
	@echo "    i.e. SERVICE_NAME=game make merge ./.stage0_template/Specifications"
...
test:
	...
	(diff -qr "$$(pwd)/.stage0_template/test_expected/" "$$HOME/tmp/testRepo/" 2>/dev/null || true) | (grep -v '\.git' | grep -v '\.DS_Store' | grep -v 'node_modules' | grep -v 'dist' | grep -v 'playwright-report' | grep -v 'test-results' || true) > /tmp/phaser_diff.txt; \
	if [ -s /tmp/phaser_diff.txt ]; then cat /tmp/phaser_diff.txt; echo "Fail: unexpected differences."; exit 1; fi; \
	echo "Done."
...
diff:
	...
	if [ -z "$$FILESPEC" ]; then \
		echo "Usage: make diff <filespec>  (e.g. make diff src/api/types.ts)"; \
		exit 1; \
	fi; \
...
take:
	...
	if [ -z "$$FILESPEC" ]; then \
		echo "Usage: make take <filespec>  (e.g. make take src/api/types.ts)"; \
		exit 1; \
	fi; \
	TEMP="$$HOME/tmp/testRepo/$$FILESPEC"; \
	EXP="$(PWD)/.stage0_template/test_expected/$$FILESPEC"; \
